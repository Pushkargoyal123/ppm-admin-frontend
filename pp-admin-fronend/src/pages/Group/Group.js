...
export default function Group() {
  ...
  const datatableData2 = leaderboardList.map((r, index) => {
    return [
      ...
      r.current_investment.toFixed(2),
      (r.totalCurrentPrice - r.current_investment).toFixed(2),
      (r.totalCurrentPrice - r.current_investment).toFixed(2),
      (r.number * 10).toFixed(2),
      r.ppm_userGroups[0].virtualAmount.toFixed(2),
      r.ppm_userGroups[0].netAmount.toFixed(2),
  ...
  const datatableData = rows.map((row, index) => {
    return [
      ...
      <Button
        onClick={() => {
          GroupMemberList(row.name, row.value, row.id);
        }}
        variant="outlined"
        color="primary"
      >
      ...
      change === index + 1 ? (
        !row.ppm_portfoliohistories.length ? (
          <>
            <input
              style={{ width: "6em", margin: "2px" }}
              onChange={(e) => setVirtualAmount(e.target.value)}
              min="0"
              type="number"
              defaultValue={row.virtualAmount}
            />
          ...
          <Tooltip title="Virtual Amount Can't Update When User Active in Group">
            <Chip
              onClick={() =>
                notifyWarning({
                  Message:
                    "Virtual Amount Can't Update When User Active in Group",
                  ProgressBarHide: true,
                })
              }
              style={{
      ...
      row.value ? (
        <Select
          ...
          value={row.status}
          onChange={(event) => {
            handleUpdate(row.id, event);
          }}
          input={<Input />}
          renderValue={(selected) => (
            <Chip
              label={selected}
              classes={{ root: classes[states[row.status.toLowerCase()]] }}
            />
          )}
        >
          {["active", "inactive", "deleted"].map((changeStatus) => (
            <MenuItem key={changeStatus} value={changeStatus}>
              <Chip
                label={changeStatus}
                classes={{ root: classes[states[changeStatus.toLowerCase()]] }}
              />
            </MenuItem>
          ))}
        </Select>
  ...
  return (
    <>
      ...
      >
        ...
        <MenuItem>
          <Button
            onClick={() => setOpenCriticalAnalysisModal(true)}
            color="primary"
          >
            Critical Analysis
          </Button>
        ...
        <MenuItem onClick={handleClose}>
          <Button onClick={() => LeaderBoardList()} color="primary">
            Leaderboard
          </Button>
        </MenuItem>
      </Menu>

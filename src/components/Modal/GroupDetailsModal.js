...
export default function GroupDetailsModal(props) {
        ...
        if (props.eventId) {
            ...
            if(data.success){
                const finalData = data.data.map(function (item, index) {
                    ...
                    item["Event Virtual Amount"] = "₹" + item.ppm_dream_nifty.virtualAmount;
            ...
            data = await getRequestWithFetch("user/getUserGroupsList?UserId=" + props.userId);
            if (data.success) {
                const finalData = data.data.map(function (item, index) {
                    item.SNO = index + 1;
                    item["Virtual Amount"] = "₹" + item.virtualAmount;
    ...
    const handleUpdateStatus = async (value, tableMeta) => {
        ...
        if(props.eventId){
            ...
            data = await postRequestWithFetch("dreamNifty/user/updateUserEventStatus", body);
        }else{
            const body = {
                status: value,
                id: selectedRow.id
